...
- `kubernetes/templates/workflows/`: サンプル Workflow
- `kubernetes/templates/events/`: Argo Events サンプル（Webhook -> Workflow）
...
```bash
kubectl create -n argo -f kubernetes/templates/workflows/hello-world.yaml
...
```bash
kubectl apply -n argo -f kubernetes/templates/workflows/cron-hello-world.yaml
...
- `kubernetes/templates/workflows/cron-hello-world.yaml` は 5 分ごとに Workflow を起動します。停止するときは `kubectl delete -n argo -f kubernetes/templates/workflows/cron-hello-world.yaml` を実行してください。
...
```bash
kubectl apply -f kubernetes/templates/workflows/webhook-message-template.yaml
```

3) EventBus / EventSource / Sensor / RBAC を作成

```bash
kubectl apply -f kubernetes/templates/events/eventbus.yaml
kubectl apply -f kubernetes/templates/events/rbac-sensor-workflow-trigger.yaml
kubectl apply -f kubernetes/templates/events/eventsource-webhook.yaml
kubectl apply -f kubernetes/templates/events/sensor-webhook-workflow.yaml
```
